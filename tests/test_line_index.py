...
def test_find_closest():
    ...
    assert idx.find_closest(1.05 + 1j, 0.01)[0] is None
    assert idx.find_closest(1.05 + 1j, 0.051) == (0, False)
    assert idx.find_closest(1.05 + 1j, 0.16) == (0, False)
    idx.pop(0)
    assert idx.find_closest(1.05 + 1j, 0.16) == (1, False)
    assert idx.find_closest(1.05 + 1j, 0.051)[0] is None
...
def test_reverse():
    ...
    assert idx.find_closest(0.1, 0.5)[0] is None
    assert ridx.find_closest(0.1, 0.5) == (0, True)
    assert ridx.find_closest(10.01, 0.5) == (0, False)
    assert ridx.find_closest(10.09, 0.5) == (1, True)
